...
/** \file   SdioDrv.c 
 *  \brief  The OMAP SDIO driver (platform and OS dependent) 
 * 
 * The lower SDIO driver (BSP) for OMAP on Linux OS.
...
int sdiodrv_dma_init(void)
{
	...
	rc = omap_request_dma(OMAP44XX_DMA_MMC5_TX, "SDIO WRITE", NULL /* sdiodrv_dma_write_cb */, &g_drv, &g_drv.dma_tx_channel);
	if (rc != 0) {
		PERR("sdiodrv_dma_init() omap_request_dma(OMAP34XX_DMA_MMC2_TX) FAILED\n");
		goto out;
	}

	rc = omap_request_dma(OMAP44XX_DMA_MMC5_RX, "SDIO READ", sdiodrv_dma_read_cb, &g_drv, &g_drv.dma_rx_channel);
	if (rc != 0) {
                      ...
                      unsigned int bMore)
{
	...
	if (iStatus != 0)
	{
		printk("CHITRA:: CMD53 read failed\n");
                       ...
                       unsigned int bMore)
{
	...
	if (iStatus != 0)
	{
		printk("CHITRA:: CMD53 write failed\n");
